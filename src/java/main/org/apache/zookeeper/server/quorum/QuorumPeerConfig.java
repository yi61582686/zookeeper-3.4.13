...
public class QuorumPeerConfig {
    ...
    throws IOException, ConfigException {
        ...
        for (Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if (key.equals("dataDir")) { // 快照文件目录
                dataDir = value;
            } else if (key.equals("dataLogDir")) { // 事务日志目录
                dataLogDir = value;
            ...
            } else if (key.startsWith("server.")) { // 解析server配置，会根据该配置决定启动方式：集群or单机
                int dot = key.indexOf('.'); // 获取逗号的位置
                long sid = Long.parseLong(key.substring(dot + 1)); // 去掉逗号之前的字符，即server.
                ...
                if (parts.length > 2){
                	electionPort=Integer.parseInt(parts[2]); // 选举的端口
                }
                if (parts.length > 3){
                    if (parts[3].toLowerCase().equals("observer")) { // 观察者。不参与选举
                        type = LearnerType.OBSERVER;
                    } else if (parts[3].toLowerCase().equals("participant")) { // 参与者，默认就是参与者。参与选举
        ...
        if (servers.size() == 0) {
             ...
             */
            if(serverGroup.size() > 0){ // 选主策略，如果是分组的话，执行使用分组的选举策略
            ...
            } else { // 如果没有使用分组，使用默认的选举策略
                /*
                 * The default QuorumVerifier is QuorumMaj
                 */

                LOG.info("Defaulting to majority quorums");
                quorumVerifier = new QuorumMaj(servers.size()); // 不包括观察者
            }

            // Now add observers to servers, once the quorums have been
            // figured out
            servers.putAll(observers); // 这里才把观察者加到servers中
